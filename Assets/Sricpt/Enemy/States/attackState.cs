...
namespace SG
{
    public class attackState : State
    {
                ...
                else if(distanceFromTarget < currentAttack.maxmumDistanceNeededToAttack)
                {
                    ...
                    && viewableAngle >= currentAttack.minimunAttackAngle)
                    {
                        if(enemyManager.currentRecoveryTime <= 0 && enemyManager.isPerformingaction == false)
                        {
                            ...
                            SetIsLeftAttack(enemyAninmatorManager);
        ...
        public void SetIsLeftAttack(EnemyAninmatorManager enemyAninmatorManager)
        {
             if(currentAttack.isLeft)
            {
                enemyAninmatorManager.anim.SetBool("isLeft",true);
            }
            else
            {
                enemyAninmatorManager.anim.SetBool("isRight",true);
            }
        }
